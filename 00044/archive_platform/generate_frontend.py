#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate complete index.html for archive management platform"""
...
function confirmDialog(msg){return new Promise(resolve=>{
  ...
  overlay.innerHTML=`<div class="modal" style="width:380px">
    <h3>确认操作</h3>
    <p style="margin-bottom:20px;color:var(--text-light)">${msg}</p>
    <div class="actions">
      <button class="btn btn-outline" id="cf-cancel">取消</button>
      <button class="btn btn-primary" id="cf-ok">确认</button>
    </div></div>`;
...
function showAddDestructionModal(){
  ...
  api('/api/archives/?status=active&size=100').then(archives=>{
    ...
    sel.innerHTML='<option value="">请选择档案</option>'+archives.map(a=>'<option value="'+a.id+'">'+a.archive_no+' - '+a.title+'</option>').join('');
  });
